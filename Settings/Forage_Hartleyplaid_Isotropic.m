...
function [S,P] = Forage_SummationGratings()
...
% --- Hartleyplaid defaults (case 10) ---
P.noisetype        = 14;      % choose Hartleyplaid
...
%         S.RandPhase = 'Randomize grating phase (1 or 0):';
        ...
        % --- Hartleyplaid (tight rectangular Hartley sublattice) ---
        P.noisetype        = 14;
        
        % Display / stimulus
        %P.noiseFrameRate   = 60;        S.noiseFrameRate   = 'Noise update rate (Hz):';
        P.noiseContrast    = 0.30;     S.noiseContrast    = 'Noise RMS contrast:';
        P.hp_K             = 6;        S.hp_K             = 'Components per frame:';
        
        % Tight grid controls
        P.hp_maxSF         = 2;        S.hp_maxSF         = 'Max spatial freq (cyc/deg):';

        % %Isotropic strided lattice
        P.hp_ns            =11;       S.hp_ns            = 'Grid points per axis (odd):';
        P.equalStride = false;     S.equalStride           = 'equal (true) or isotropic (false) stride mode:';

        % full integer lattice
        % P.hp_ns            = Inf;       S.hp_ns            = 'Grid points per axis (odd):';
        % P.equalStride = true;      S.equalStride           = 'equal (true) or isotropic (false) stride mode:';

        % equal cycles per screen HxW gives different cyc/deg unless you have a square screen


        % (Optional) override: leave empty to auto-pick from ns/maxSF
        P.hp_strideKx      = [];       S.hp_strideKx      = 'Stride kx (leave empty for auto):';
        P.hp_strideKy      = [];       S.hp_strideKy      = 'Stride ky (leave empty for auto):';
        P.noiseFrameRate   = 60;      S.noiseFrameRate  = 'Hz (200 ms static frames)';
        %P.noiseContrast    = 0.3;     S.noiseContrast   = 'overall RMS target (scaled per component)';
        P.noiseorinum      = 6;       S.noiseorinum     = 'orientations (0:30:150)';
        P.orioffset        = 0;       S.orioffset       = 'degrees, optional';
        P.spfnum           = 5;       S.spfnum          = 'number of spatial frequencies';
        P.spfmin           = 1.5;     S.spfmin          = 'cpd (min)';
        % P.spfmax           = 6;       S.spfmax          = 'cpd (max)';
        % P.hp_K             = 6;       S.hp_K            = 'components per frame';
        P.hp_M             = 8;       S.hp_M            = 'phase bins per component';
        
        % Optional probe mode (Δφ sweep for selected pairs)
        P.hp_probeMode         = false;
        P.hp_probePairs        = [];       % rows of [linearIdx_i, linearIdx_j] over O×S
        P.hp_probeDeltaPhases  = [];       % defaults to 0:2π/M
