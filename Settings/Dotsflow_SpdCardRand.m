...
function [S,P] = Dotsflow_SpdCardRand()
% Settings for DotsflowSpdCardRand
% Last Modification: May 05, 2025 - PSC
% size = 35pix @ 50cm VD 
...
S.finish = 40; % 40rep = 33.3min

% PROTOCOL PREFIX
S.protocol = 'DotsflowSpdCardRand';
% PROTOCOL PREFIXS
...
S.protocolTitle = 'RandSpd cardinal dirs Flow';
...
P.size = 35;
S.size = 'Dot size (pix)'; % 0.8434 visual deg @ 50cm vd 
P.speed = [2 4 8 16 32 64];
S.speed = 'Dot motion speeds (deg/s)';
P.nspds = length(P.speed);
S.nspds = 'number of speeds';

P.direction = [0 90 180 270];
S.direction = 'Dot motion directions (deg)';
P.ndirs = length(P.direction);
S.ndirs = 'number of directions';

[P.spdmesh,P.dirmesh] = meshgrid(P.speed,P.direction);
...
P.trialdur = 2.0*24; % this is also the stimulus duration 
