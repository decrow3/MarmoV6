classdef densegabornoise < stimuli.stimulus
    ...
    methods
        function obj = densegabornoise(winPtr, varargin)
            ...
            %Pregenerate matrix of settings for 2500 frames, these will be
            %looped over 
        end
...
%                 obj.mypars(3,:) = (2*obj.mypars(3,:).*obj.pixPerDeg+1)/4; % scale to pixels
                ...
                SFdeg=obj.mypars(2,:).*obj.pixPerDeg;
                sizedeg=1.5./SFdeg+((rnd(4,:)-.5)*.25*obj.scaleRange); %1.5 cycles +random +-.25 of scalerange
               obj.mypars(3,:) = (2*sizedeg.*obj.pixPerDeg+1)/4; % scale to pixels
