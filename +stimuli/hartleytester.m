...
S.gamma=1;
%S.screenRect = [0 0 640 480];
A = marmoview.openScreen(S);
inver_gamma=1/S.gamma; 

% Build a hartleyplaid in PROCEDURAL mode (fast):
...
Sumat = sum(atoms.^2, 3);
flatness = std(Sumat(:)) / mean(Sumat(:));
fprintf('Flatness ratio std/mean = %.2e (want ~0)\n', flatness);

% Optional: visualize
figure(99); imagesc(Sumat); axis equal tight; colormap(gray);
