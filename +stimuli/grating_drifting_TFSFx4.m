classdef grating_drifting_TFSFx4 < stimuli.stimulus
    ...
    properties
        ...
        minTF % lowest TF
        nTFs % number of temporal frequencies, by double from minTF
    ...
    methods
        function obj = grating_drifting_TFSFx4(winPtr, varargin)
            % The constructor
            ...
            %ip.addParameter('tfs', 5) % phase shift per frame
            ip.addParameter('minTF',1) % lowest TF
            ip.addParameter('nTFs',2) % number of temporal frequencies, by double from minTF
            ...
            obj.spatialFrequencies = obj.minSF * 4.^(0:obj.numOctaves-1);
            obj.tfs = obj.minTF * 4.^(0:obj.nTFs-1);
            obj.phase = 0;
...
%                     obj.cpd = 0; % grating is blank (CPD = 0)
                ...
                elseif obj.contrast == 0 % stimulus was just off
                    ...
                    obj.tf = randsample(obj.rng, obj.tfs, 1); %cycles per second
                    obj.dphase = obj.tf/obj.frameRate*360; %*obj.cpdphase=speed*sf; speed=tf/sf;
