classdef dotspatial_MTImaging < stimuli.stimulus
    ...
    methods
        function obj = dotspatial_MTImaging(winPtr, varargin)
            ...
            ip.addParameter('speed',16); % deg./s
        ...
        function afterFrame(obj,xshift,yshift,flowsign,init)
        
            obj.moveDots(xshift,yshift,flowsign,init)
        ...
        function initDots(obj, idx, xshift, yshift)
            ...
            obj.dy(idx) =  - obj.speed *obj.pixPerDeg* 0.0042; % 1./frameRate
        ...
        function moveDots(obj, xshift, yshift,flowsign,init)
            
            obj.dx= xshift;
            %obj.dy = yshift;
            
            obj.dy =  flowsign*obj.speed *obj.pixPerDeg* 0.0042;
            ...
            winC = [round(win(1)/10), round(win(2)/20)];
            
            idx = find(obj.x > win(1) | obj.x < -win(1) | obj.y > win(2)|obj.y < -win(2) |obj.yr <= 0)';
            idxC = find((obj.y<= winC(2)&obj.y>=-winC(2))|obj.yr <= 0)';
            if ~isempty(idx)
            ...
            if flowsign>0
                if ~isempty(idxC)
                    % (re-)place dots randomly within the aperture
                    obj.initDots(idxC,xshift,yshift);
                end
            end 
