classdef PR_oneDStim < handle
  ...
  properties (Access = private)
    ...
    Bars;             % object that stores bar textures for use
    Bars0;             % object that stores bar textures for use
    Bars1;             % object that stores bar textures for use
    Bars2;             % object that stores bar textures for use
    StartTex;
  ...
  methods (Access = public)
    ...
    function initFunc(o,S,P);
       ...
       o.Bars0 = stimuli.oneDnoise(o.winPtr,'bkgd',S.bgColour,'gray',false); % 1D noise as probe
       %o.Bars.prctgray = 33.33;
       o.Bars0.sparsity = 0;
       o.Bars0.contrast = P.probecon;
       o.Bars0.texnum = 1;
       o.Bars0.Ntex      = (S.frameRate./P.nFramesPerStim)*P.nSecondsPreGen; %1 trials at 120fps
       o.Bars0.barwidth  = round(P.barwidth*S.pixPerDeg);
       o.Bars0.pxradius    = round(P.proberadius*S.pixPerDeg);
       o.Bars0.prefori   = P.prefori;
       o.Bars0.pixPerDeg = S.pixPerDeg;
       
       o.Bars0.makeTex();
       
       o.Bars1 = stimuli.oneDnoise(o.winPtr,'bkgd',S.bgColour,'gray',false); % 1D noise as probe
       %o.Bars.prctgray = 33.33;
       o.Bars1.sparsity = 1;
       o.Bars1.contrast = P.probecon;
       o.Bars1.texnum = 1;
       o.Bars1.Ntex = (S.frameRate./P.nFramesPerStim)*P.nSecondsPreGen; %1 trials at 120fps; %1 trials at 120fps
       o.Bars1.barwidth  = round(P.barwidth*S.pixPerDeg);
       o.Bars1.pxradius    = round(P.proberadius*S.pixPerDeg);
       o.Bars1.prefori   = P.prefori;
       o.Bars1.pixPerDeg = S.pixPerDeg;
       
       o.Bars1.makeTex();
       
       
      o.Bars2 = stimuli.oneDnoise(o.winPtr,'bkgd',S.bgColour,'gray',false); % 1D noise as probe
       %o.Bars.prctgray = 33.33;
       o.Bars2.sparsity = 2;
       o.Bars2.contrast = P.probecon;
       o.Bars2.texnum = 1;
       o.Bars2.Ntex = (S.frameRate./P.nFramesPerStim)*P.nSecondsPreGen; %1 trials at 120fps; %1 trials at 120fps
       o.Bars2.barwidth  = round(P.barwidth*S.pixPerDeg);
       o.Bars2.pxradius    = round(P.proberadius*S.pixPerDeg);
       o.Bars2.prefori   = P.prefori;
       o.Bars2.pixPerDeg = S.pixPerDeg;
...
%        for kk = 1:o.oriNum
%            %*******
%            o.hProbe{kk} = stimuli.barcode(o.winPtr,'bkgd',S.bgColour,'gray',false); % 1D noise as probe
%            o.hProbe{kk}.transparent = -P.probecon;  % blend in proportion to gauss
%            o.hProbe{kk}.gauss = true;
%            o.hProbe{kk}.pixperdeg = S.pixPerDeg;
%            o.hProbe{kk}.pxradius = round(P.proberadius*S.pixPerDeg);
%           
...
%        end
       ...
       if (o.noisetype == 1) % use Hartley background
           ...
           for k = 1:o.noiseNum
               ...
               if isinf(P.noiseradius)
                   o.hNoise{k}.pxradius = Inf;    %fill entire screen
                   o.hNoise{k}.screenRect = S.screenRect;
               else
                   o.hNoise{k}.pxradius = round(P.noiseradius*S.pixPerDeg);
               end
               %*********
       ...
       if (o.noisetype == 2) % use Spatial background
           ...
           if (0) % comment for now, using ovals instead
             for k = 1:o.noiseNum
               o.hNoise{k} = stimuli.grating(o.winPtr);  % grating probe
               o.hNoise{k}.pxradius = round((P.snoisediam/2)*S.pixPerDeg);
...
%         end
    ...
    function reset_probe_location_and_texture(o,firstime)
        ...
        position = [(o.S.centerPix(1) + round(o.PosList(1)*o.S.pixPerDeg)),...
                                        (o.S.centerPix(2) - round(o.PosList(2)*o.S.pixPerDeg))];
        o.Bars.position=position;
%         o.Bars0.position=position;
%         o.Bars1.position=position;
%         o.Bars2.position=position;
        %**************
    end
    ...
    function P = next_trial(o,S,P);
         ...
         %Fill in some hidden parameters
           ...
           P.faceradius     = P.Radius;
           P.proberadius    = P.Radius; 
           P.fixRadius      = P.Radius*.85; %just inside
          
           o.P = P;
          o.error = 0;
          o.FrameCount = 0;
          o.PFrameCount = 0;
          o.nFramesPerStim = P.nFramesPerStim;
          
          o.Bars.sparsity=randi(3,1)-1;
          
          %Select textures for next trial based on sparseness
          switch o.Bars.sparsity
              case 0; o.Bars=o.Bars0;
              case 1; o.Bars=o.Bars1;
              case 2; o.Bars=o.Bars2;
          end
    ...
    function [drop,faceitem] = updateProbes(o,xx,yy,currentTime)
        ...
        if (o.state > 0)
            ...
            if (o.MovStep > 0)
                ...
                position = [(o.S.centerPix(1) + round(newPos(1)*o.S.pixPerDeg)),...
        ...
        if (o.state == 1)  % if state == 2 or 3, then no stim visible to check
            ...
            if (dist < o.P.fixRadius)
               ...
               if (o.FixTime == 0)
                 o.hProbe{o.oriNum}.position = o.Bars.position;
                 o.targOri = o.oriNum;
               end
               %*****
            else
            ...
            if (itfix > 0)
                if ~o.FixTime  %first detection, set the time
                   ...
                   if ( (currentTime - o.FixTime) > o.P.holdDur )
                         ...
                         o.Faces.position = o.Bars.position; % set at face at target 
                         %**********
                         o.reset_probe_location_and_texture(0); % make it jump, firstime=1
        ...
        if (o.PFrameCount < o.MaxFrame)
            ...
            %DPR
            %Update the probe to next texture based on number of frames shown            
%             if isempty(o.Bars.texnum)
%                 o.Bars.texnum = o.StartTex;
%             else
%                 o.Bars.texnum  = o.Bars.texnum +1;
%             end
            %disp(o.Bars.texnum )
            
            kk = ceil(o.PFrameCount / o.nFramesPerStim);
            
            
            o.Bars.texnum   = o.StartTex + kk;%o.Bars.texnum +1; %randi(o.Bars.rng, o.Bars.Ntex);  
            
%             if isempty(o.Bars.texnum)
%                 o.Bars.texnum = o.StartTex;
%             else
%                 o.Bars.texnum=o.Bars.texnum+ kk;
%             end
            ...
            if (drop == 1)  % will be moving to state 2 or 3
                   ...
                   o.ProbeHistory(o.PFrameCount,5) = NaN;
                   o.ProbeHistory(o.PFrameCount,6) = NaN;
                   
                   o.BarHistory{o.PFrameCount} = NaN;
                   %o.TexHistory(o.PFrameCount,:) = NaN;
            else
                if (o.state < 2)
                   ...
                   o.ProbeHistory(o.PFrameCount,5) = o.Bars.texnum;
                   o.ProbeHistory(o.PFrameCount,6) = o.Bars.sparsity;
                   ...
                   o.BarHistory{o.PFrameCount} =o.Bars.saveline(o.Bars.texnum,:);
    ...
    function drop = state_and_screen_update(o,currentTime,x,y,varargin) 
        ...
        % Draw probe stimuli
        if (o.state < 2)
           o.Bars.beforeFrame();  % only one target now
        end
    ...
    function PR = end_plots(o,P,A)   %update D struct if passing back info
        ...
        if o.FrameCount == 0
            ...
            PR.BarHistory = o.BarHistory{1:o.PFrameCount};
            %PR.TexHistory= o.TexHistory(1:o.PFrameCount,:,:);
        end
