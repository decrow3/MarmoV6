classdef PR_ForageMT_Imaging < handle
  ...
  properties (Access = private)
    ...
    MaxFrame = (240*24); % twenty second maximum
    TrialDur = 24;      % store internally the trial duration (make less than 20)
    cycleTime =[];
    stimEnds =[];
  ...
  methods (Access = public)
      function o = PR_ForageMT_Imaging(winPtr)
      o.winPtr = winPtr;
    ...
    function initFunc(o,S,P);
       ...
       if (o.noisetype == 5) % use Spatial background
           ...
           for kk = 1:o.noiseNum
               ...
               %*******
               spd = 1+mod((kk-1),P.snoisespds); % randi(P.snoisespds);
               noisespeed = P.snoisebase * (sqrt(2)^(spd-1));
               o.hNoise(7,kk) = spd;
               %*******
            ...
            for i = 1:length(o.stimEnds)
                if (currentTime-o.startTime)<=o.cycleTime(o.stimEnds(i))&&(currentTime-o.startTime)...
                    ...
                    if (o.noisetype == 5)
                        ...
                        o.noiseFrame(o.FrameCount,:)  =[currentTime-o.startTime,o.nf];
                        %**********
                    end
                    %**********
                end
...
%         %DPR - 5/5/2023
    ...
    function PR = end_plots(o,P,A)   %update D struct if passing back info
        ...
        if o.FrameCount == 0
            ...
            PR.noiseFrame = o.noiseFrame(1:o.FrameCount,:);
        end
