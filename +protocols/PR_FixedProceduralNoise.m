classdef PR_FixedProceduralNoise < protocols.protocol
  % Noise protocol that updates only during saccades, still generate foraging
  % probe for reward/engagement?
  %
  ...
  methods (Access = public)
    function o = PR_FixedProceduralNoise(winPtr)
...
%                      o.NoiseHistory(o.FrameCount,3) = o.hNoise.mypars(2); 
    ...
    function drawNoise(o,xx,yy,currentTime)
         if (o.FrameCount < o.MaxFrame)
             switch o.noisetype
...
%                      %**********
    ...
    function [drop,faceitem] = updateProbes(o,xx,yy,currentTime)
        ...
        if (o.state == 2) %Only check on saccade  % if state == 2 or 3, then no stim visible to check
            %**********************************************************


            %Since we change the stimuli on fixation, for the following to
            %work we need to make o.FixTime=0
            %o.Holdur=0;
    ...
    function drop = state_and_screen_update(o,currentTime,x,y, varargin) 
        ...
        % NOW STATE DEPENDANT, ONLY UPDATE THE TEXTURES IF THEY ARE CHANGED
        % DURING A SACCADE
