    ...
    function drop = state_and_screen_update(o,currentTime,x,y, varargin) 
        ...
        if currentTime > o.startTime + o.TrialDur
            ...
            if isa(o.hNoise, 'stimuli.hartleyplaid_procedural')
                %Frame control expects [0 255] colorrange
                try Screen('ColorRange', o.hNoise.winPtr, o.hNoise.oldmaximumvalue, o.hNoise.oldclampcolors, o.hNoise.oldapplyToDoubleInputMakeTexture); catch, end
            end
            return
        end
        %***********************
        ...
        if (o.state == 3) 
            % if isfield(o.S,'stereoMode') && o.S.stereoMode>0
                ...
                o.Faces.beforeFrame();
            % else
            %     o.Faces.beforeFrame();  % only one target now
            % end
        end
        %****************************************   
...
%         %DPR - 5/5/2023
    ...
    function Iti = end_run_trial(o)
        ...
        % if isa(o.hNoise, 'stimuli.hartleyplaid_procedural')
        %     Screen('FillRect', o.winPtr, o.P.bkgd);
        % end
    end
