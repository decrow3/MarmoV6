...
switch RigName
    ...
    case 'Laptop'
        ...
        S.frameRate = 60;                  % Frame rate of screen in Hz
        S.screenRect =[0 0 960 540];%[0 0 960*2 540*2];%      % Screen dimensions in pixels
        S.screenWidth = 15;                 % Width of screen (cm)
        S.centerPix =  [S.screenRect(3)/2 S.screenRect(4)/2];%[480 270];           % Pixels of center of the screen
        S.guiLocation = [1000 100 890 660];
        S.bgColour = 127; % 186 if not gamma corrected
        S.output_arduino.port = 'COM3';

        S.screenDistance = 30; %57;         % Distance of eye to screen (cm)
        S.pixPerDeg = PixPerDeg(S.screenDistance,S.screenWidth,S.screenRect(3));
        S.DummyScreen = true; % TODO: remove this parameter (it should be covered by screen Rect. Is it?)

        S.gamma = 1;
        S.screenDistance = 30;              % Distance of eye to screen (cm)
        S.pixPerDeg = PixPerDeg(S.screenDistance,S.screenWidth,S.screenRect(3));
        ...
        %Photodiode patch parameters
        S.photodiode.rect =[0 0 100 100];%[1920-100 1080-100 1920 1080];%[0 0 100 100];%
        S.photodiode.init = 0;
        S.photodiode.flash = 255;
        S.photodiode.TF   = 2;
