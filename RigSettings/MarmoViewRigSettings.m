...
switch RigName
    ...
    case 'testTDM'
        ...
        S.inputs = {'eyetrack_OpenIris'};
         %S.inputs = {'eyetrack_OpenIris_pupil_right'}; %eyetrack_dummy; eyetrack_OpenIris
        ...
        S.feedback_newera.pumpDiameter = 38.1; % updated by PSC 2025 May 1st% internal diameter of the juice syringe (mm)
        ...
        S.treadmill_arduino.rewardDist = 94.25/5;%was 94.25/8 08-01-2024;  %PSC changed to 94.25 06052023
        S.treadmill_arduino.rewardProb = 1; % 1

        S.trackball_mouseDevice.baud = 115200;
        S.trackball_mouseDevice.port = '/dev/ttyACM0';
        S.trackball_mouseDevice.scaleFactor = (94.25/5000); % circimference of wheel over ticks per rev
        S.trackball_mouseDevice.rewardMode = 'distProb';
        S.trackball_mouseDevice.rewardDist = 94.25/5;%was 94.25/8 08-01-2024;  %PSC changed to 94.25 06052023
        S.trackball_mouseDevice.rewardProb = 1; % 1
        ...
        %S.gamma = 2.5554; %updated on Oct 9, 2023 
        S.gamma = 2.166; %updated on Oct 23, 2023 ?? 
        ...
        % [cal, cals, fullFilename] = LoadCalFile('CalTDM0703') Todo
        % [oldtable, success] = Screen('LoadNormalizedGammaTable', windowPtrOrScreenNumber, cal.gammaTable) 

        S.screenDistance = 63.5;%50.5;%67.5; %[48,62] eye-track training;%64.25 V1/V2; 28.6900MT/MST   % Distance of eye to screen (cm)
        S.pixPerDeg = PixPerDeg(S.screenDistance,S.screenWidth,S.screenRect(3));
        
        %Photodiode patch parameters
        S.photodiode.rect =[S.screenRect(3)-150  S.screenRect(4)-150 S.screenRect(3) S.screenRect(4)];%[0 0 100 100];%
