% wrapper class for trackball 
% 03/08/2025 - Penny Shuyi Chen 
classdef trackball_mouseDevice < matlab.mixin.Copyable
    ...
    methods
        function self = trackball_mouseDevice(varargin) % h is the handle for the marmoview gui
           ...
           % CURRENT: testing for only one mouse-like device 
            [self.mouseIndices, self.productNames, self.allInfo] = GetMouseIndices('masterPointer');
    ...
    methods (Access = public)
        ...
        function out = afterFrame(self, currentTime, rewardState)
            ...
            [countX,countY, timestamp] = self.readCount();
            
            self.locationSpace(self.frameCounter, 2) = timestamp;
            if ~isnan(countX)|~isnan(countY)
                %self.locationSpace(self.frameCounter,3:4) = count * [1 self.scaleFactor];
                self.locationSpace(self.frameCounter,3) = countX * [1];
                self.locationSpace(self.frameCounter,4) = countY * [1];
...
%                         tmp = double(rand < self.rewardProb);
        ...
        function [countX,countY, timestamp] = readCount(self)
...
%             end
                [countX,countY] = GetMouse([],self.mouseIndices); 
                timestamp = GetSecs();
        end
