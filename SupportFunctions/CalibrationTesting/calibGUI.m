...
classdef calibGUI < handle
    ...
    methods (Access = public)
        ...
        function initialize(obj)
            ...
            validTrials = cellfun(@(x) strcmp(x.PR.name,'FaceCal'),obj.Exp.D); %io.getValidTrials(obj.Exp, 'FaceCal');
            tstart = (cellfun(@(x) x.STARTCLOCKTIME, obj.Exp.D(validTrials)));
            tstop = (cellfun(@(x) x.ENDCLOCKTIME, obj.Exp.D(validTrials)));
            
            if ~isfield(obj.Exp,'vpx')
                    obj.Exp.vpx.raw=cell2mat(cellfun(@(x) x.eyeData,obj.Exp.D,'UniformOutput',false));
                    obj.raw_field='raw';
            end
            
            eyeTime = (obj.Exp.vpx.(obj.raw_field)(:,1));
            ...
            %obj.spd = [0; hypot(diff(obj.xy(:,1)),diff(obj.xy(:,2)))];
...
%             ix = all(abs(zscore(obj.xy(ix,:)))<1,2); % remove outliers
            ...
            %Use online calib first, throwout anything way offscreen
            if isfield(obj.Exp,'vpx')
                ix = ix & (hypot( obj.Exp.vpx.smo(obj.validix,2) , obj.Exp.vpx.smo(obj.validix,3)))<25;
            end
...
%             ix = ix & ( obj.spd / median(obj.spd) < .1); % find fixations
            ...
            imagesc(obj.H.xpos, xax, yax, sqrt(I)); axis equal tight
            hold(obj.H.xpos, 'on')
